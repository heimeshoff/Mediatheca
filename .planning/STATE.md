...
**Current Phase:** 7B (Games Dashboard + Steam Integration) — In progress
**Current Task:** REQ-210 through REQ-213 implemented

## Recent Progress

- **2026-02-17**: Implemented REQ-210 through REQ-213 (Steam-first metadata, expandable descriptions, external links, play mode categories)
  - Steam.fs: Added `getSteamStoreDetails` function — fetches short_description, detailed_description, about_the_game, website, categories from Steam Store API
  - Games.fs: Added 4 new events (Game_short_description_set, Game_website_url_set, Game_play_mode_added, Game_play_mode_removed), new state fields (ShortDescription, WebsiteUrl, PlayModes), commands, serialization
  - GameProjection.fs: Added short_description, website_url, play_modes columns to game_detail table
  - Shared.fs: Added ShortDescription, WebsiteUrl, PlayModes to GameDetail DTO; addGamePlayMode/removeGamePlayMode API endpoints
  - Api.fs: Steam import now fetches Steam Store details for description, short description, website, and play modes; HTML tags stripped from Steam descriptions; name-matched games also enriched with Steam Store data
  - GameDetail Views: Expandable description (short + "Read more" / "Show less"), External Links card on right sidebar (Steam Store + official website), Play Modes section with removable pills and custom input
  - Build: 191 tests (189 passed, 2 pre-existing integration errors)
- **2026-02-17**: Added REQ-210 through REQ-213 to Phase 7B — Steam-first metadata, expandable descriptions, external links, play mode categories. Added REQ-214 (daily Steam playtime deltas for year-in-review) to v2 backlog — needs modeling conversation for data structure before implementation.
...
4. **REQ-210: Steam-first metadata** — prefer Steam Store API for descriptions, fall back to RAWG
5. **REQ-211: Expandable descriptions** — short description with ellipsis, "read more" for full text
6. **REQ-212: External links** — Steam page + official website links on game detail page
7. **REQ-213: Play mode categories** — fetch from Steam Store, multi-select, user-extendable
8. Polish Game detail page (visual refinements, mobile responsiveness)
9. Fix pre-existing integration test failures (missing friend_list table in test setup)
