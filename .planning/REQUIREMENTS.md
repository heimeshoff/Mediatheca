...
- [x] REQ-210: Steam-first metadata — when importing a game that exists on Steam, fetch description and metadata from Steam Store API first, fall back to RAWG only if Steam data is unavailable. [Phase 7B]
- [x] REQ-211: Expandable game descriptions — show Steam's short description by default with ellipsis; "read more" expands to full description. [Phase 7B]
- [x] REQ-212: External links on game detail page — links to Steam store page and original website (if available in data), displayed on the right side of the game detail page. [Phase 7B]
- [x] REQ-213: Play mode categories — fetch singleplayer/multiplayer/co-op/online multiplayer/online co-op/local multiplayer/local co-op/remote play together from Steam Store API. Display as multi-select on game detail page. User-extendable with custom categories (e.g., "couch gaming"). [Phase 7B]
...
- [ ] REQ-214: Daily Steam playtime delta tracking — daily fetch from Steam for last-2-weeks playtime, calculate daily deltas, store in database for year-in-review generation
