...
- **Adapter** — module that owns one external system (`Tmdb.fs`, `Rawg.fs`, `Steam.fs`, `HowLongToBeat.fs`, `Jellyfin.fs`, `CinemarcoImport.fs`). Translates DTOs ↔ domain language. `SteamConnect.fs` is a narrow SteamKit2-only satellite of `Steam.fs`, scoped to the one-time QR login below — it never touches the ongoing HTTP-only adapter surface.
