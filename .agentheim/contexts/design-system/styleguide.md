...
> **Status:** Tokens & type foundation **shipped** (design-system-r7k2m: palette, typography, glass re-tint). Component patterns & motion **shipped** (design-system-h3q8n: § 1.3–1.6 tokens, § 3.1 velvet card, § 3.3 media-chrome glass, § 4 component patterns, § 4 Motion). Sidebar nav **shipped** (design-system-t4b9k: layered rail, ivory active tab, concave corner-notch boundary — ADR-0013). Adopted from the *Mediatheca design brief* (Claude Design), turn 3, option **3a** — the "Velvet Lobby" desktop dashboard. The two gating decisions below (glassmorphism coexistence, theme replace-in-place) were applied as recommended defaults while the user was away and are **flagged for re-confirm**; full human sign-off on the redesign is still open.
...
| Active nav surface (ivory placard, not `--color-active-nav-surface` burgundy per the brief — see § 4 Sidebar nav) | `oklch(0.94 0.02 75)` | Shipped — `--color-nav-active-bg` (design-system-t4b9k, ADR-0013) |
| Active nav ink (flips to dark burgundy on the ivory tab) | `oklch(0.20 0.03 25)` | Shipped — `--color-nav-active-ink` (design-system-t4b9k) |
| Active nav icon (gold, distinct from the ink-colored label) | `oklch(0.55 0.16 55)` | Shipped — `--color-nav-active-icon` (design-system-t4b9k) |
...
### Sidebar nav (desktop rail) — *Implemented (design-system-t4b9k)*
Deep rail (`oklch(0.14 0.025 20)`, `base-300`), `w-64`, `line`-toned right border. **Wordmark at top** (`Components/Sidebar.fs`): "Media" (Instrument Serif, ink) + italic gold "*theca*" — the brief's 3a treatment (tagline omitted), shipped by design-system-r7k2m.

**Layered rail.** The item list is a **top group** (Dashboard / Movies / TV Series / Games / Catalogs / Friends) and a **bottom group** (Events / Settings) pinned to the foot of the rail via `margin-top: auto`. The active item is its own **raised ivory layer**, distinct from the flat rail surface behind it, not a highlight painted on top of it — this is the "layered" affordance.

**Active tab.** Ivory placard surface (`--color-nav-active-bg`, `oklch(0.94 0.02 75)`, Velvet Lobby gold family — **not** literal white), dark-burgundy ink (`--color-nav-active-ink`, `oklch(0.20 0.03 25)`), gold icon (`--color-nav-active-icon`, `oklch(0.55 0.16 55)`), raised via `--shadow-nav-active`. Reads as a lit lobby placard against the deep rail. This diverges from the brief's own dir-3a active state (a *burgundy* fill + `--ring-active` inset-left gold bar) per an explicit user override during refinement — see ADR-0013.

**Curved-corner boundary.** The active tab bleeds to the rail's right edge (cancelling the nav gutter) and the rail/content boundary sweeps concavely around its top and bottom corners via two corner-notch pseudo-elements (`--nav-notch-size`, radial-gradient hard-edge masks) — the tab appears to join the content edge rather than sit behind a straight border. Technique detail and rationale: ADR-0013.

Inactive items: muted ink, no fill, subtle hover background (`hover:bg-base-300/50`) — no left-edge glow bar (the old `.nav-glow` treatment is retired; it's fully superseded by the ivory active layer).

`.nav-item` / `.nav-item-active` / `.nav-item-active-icon` (`index.css`) + `DesignSystem.navItemClass isActive` / `navItemActiveIconClass` / `navGroupTop` / `navGroupBottom` (`DesignSystem.fs`). Specimen: StyleGuide "Velvet Lobby Patterns" § Sidebar Nav.
