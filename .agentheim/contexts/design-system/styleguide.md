...
> **Status:** Tokens & type foundation **shipped** (design-system-r7k2m: palette, typography, glass re-tint). Component patterns & motion **shipped** (design-system-h3q8n: § 1.3–1.6 tokens, § 3.1 velvet card, § 3.3 media-chrome glass, § 4 component patterns, § 4 Motion). Sidebar nav **shipped** (design-system-t4b9k: layered rail; active-tab treatment reverted to dir 3a's own burgundy fill + gold inset-left bar by design-system-grtw7 — ADR-0014 supersedes ADR-0013's ivory placard + concave corner-notch). Adopted from the *Mediatheca design brief* (Claude Design), turn 3, option **3a** — the "Velvet Lobby" desktop dashboard. The two gating decisions below (glassmorphism coexistence, theme replace-in-place) were applied as recommended defaults while the user was away and are **flagged for re-confirm**; full human sign-off on the redesign is still open.
...
| Component patterns (§ 4: hero card, filmstrip, secondary card, In-focus frame, status badges, progress meters, star rating, section header, list row) | **Shipped** | Typed `DesignSystem.fs` compositions + live StyleGuide specimens — design-system-h3q8n. In-focus badge sweep + poster frame animation fixed in design-system-bky6v (see § 4 Status badges, § 4 Poster grid). Compact on-poster "✦ Focus" pill (`inFocusPill`) shipped in design-system-fq3vp |
...
| Active nav fill (dir 3a's own burgundy — see § 4 Sidebar nav) | `oklch(0.22 0.035 25)` | Shipped — `--color-nav-active-fill` (design-system-grtw7, ADR-0014; supersedes the ADR-0013 ivory placard) |
| Active nav icon (gold, distinct from the muted label ink) | `oklch(0.80 0.12 82)` | Shipped — `--color-gold` (design-system-grtw7) |
| Inactive nav icon (muted, distinct shade from the inactive label) | `oklch(0.45 0.03 35)` | Shipped — `--color-nav-icon-muted` (design-system-grtw7) |
| Bottom-group nav muted (Events/Settings label + icon) | `oklch(0.55 0.02 40)` | Shipped — `--color-nav-bottom-muted` (design-system-grtw7) |
...
### Sidebar nav (desktop rail) — *Implemented (design-system-t4b9k, active tab reverted to dir 3a by design-system-grtw7)*
Deep rail (`oklch(0.14 0.025 20)`, `base-300`), `w-64` (256px — dir 3a's own 216px mockup proportion deliberately not adopted), `line`-toned right border. **Wordmark + tagline at top** (`Components/Sidebar.fs`): "Media" (Instrument Serif, ink) + italic gold "*theca*" (shipped by design-system-r7k2m), plus dir 3a's tagline underneath — "Where entertainment lives", 8.5px, `letter-spacing: 0.26em`, uppercase, `--color-ink-faint` (`oklch(0.52 0.04 45)`), `margin-top: 3px` (design-system-grtw7).

**Layered rail.** The item list is a **top group** (Dashboard / Movies / TV Series / Games / Catalogs / Friends) and a **bottom group** (Events / Settings) pinned to the foot of the rail via `margin-top: auto`.

**Active tab (dir 3a, design-system-grtw7).** Burgundy fill (`--color-nav-active-fill`, `oklch(0.22 0.035 25)`) with a gold inset-left bar (`--ring-active`, `inset 2px 0 0 oklch(0.80 0.12 82)`) and a gold icon (`--color-gold`) — dir 3a's own active-tab treatment. This **supersedes ADR-0013's ivory placard + concave corner-notch boundary**: the user, re-asked at refinement after seeing the shipped ivory tab running, chose to abandon that compromise and return to the brief's own burgundy spec. See ADR-0014 (supersedes ADR-0013).

**Item metrics (dir 3a).** Labels 13px, semibold only when active; icons 12px, muted (`--color-nav-icon-muted`, `oklch(0.45 0.03 35)`) by default, gold when active; item padding `9px 12px`; radius 8px; list gap 2px; list side-padding 12px. **Bottom group** (Events/Settings) is one step smaller — 12px labels, 11px icons, `--color-nav-bottom-muted` (`oklch(0.55 0.02 40)`).

Inactive items: muted ink (`--color-ink-muted`), no fill, subtle hover background (`hover:bg-base-300/50`) — no left-edge glow bar (the old `.nav-glow` treatment remains retired).

No profile chip — the single-user app has no profile/auth concept for dir 3a's "Jonas" avatar chip to represent.

`.nav-item` / `.nav-item-active` / `.nav-item-inactive` / `.nav-item-icon` / `.nav-item-active-icon` / `.nav-group-bottom` (`index.css`) + `DesignSystem.navItemClass isActive` / `navItemIconClass` / `navItemActiveIconClass` / `navGroupTop` / `navGroupBottom` / `navTagline` (`DesignSystem.fs`). Specimen: StyleGuide "Velvet Lobby Patterns" § Sidebar Nav.
...
### Poster grid (3c list page) — In-focus frame + on-poster pill *Implemented (design-system-h3q8n, animated design-system-bky6v, pill design-system-fq3vp)*, page chrome *not yet implemented*
Poster-grid list page with filter pills; **"In focus" items get the gold frame** — an animated sweeping gold-gradient border + gold glow (not a static ring) — **plus** a compact solid-gold "✦ Focus" pill on the artwork itself.
...
**On-poster pill (design-system-fq3vp):** the reviewed 3c direction's distinct compact on-artwork pill ("✦ Focus", 8.5px/700/0.18em tracking, dark ink on solid gold `oklch(0.84 0.11 85)`, top-left on the poster) is now shipped as `DesignSystem.inFocusPill` / `.in-focus-pill` in `index.css` — a genuinely separate composition from `statusBadge InFocus` (list rows, hero, detail), free to diverge. It renders as a sibling of `inFocusFrame` inside a `position: relative` wrapper (not nested inside `.in-focus-frame-inner`, so it isn't clipped). Any future poster-grid page chrome should compose `inFocusFrame` + `inFocusPill` together for "In focus" items — `inFocusPill` (not `statusBadge InFocus`) is the poster-grid on-artwork badge. Specimen: StyleGuide § In-Focus Pill (compact on-poster badge). Deliberately a **solid** fill, not `.gold-sweep` — see § 4 Motion discipline below for the rationale.
...
- **`inFocusPill` is deliberately solid, not swept (design-system-fq3vp).** The compact on-poster "✦ Focus" pill (§ 4 Poster grid) does **not** carry `.gold-sweep` / `goldLeafSweep`, and introduces no new keyframe. Rationale: (1) *motion economy* — the pill always renders on top of a poster wrapped in the animated `inFocusFrame`, whose sweeping border sits directly behind it; a second sweep on an ~8.5px pill competes with the frame's sweep and reads as jitter rather than life, so solid pill + animated frame keeps exactly one motion focal point per poster; (2) *perceptibility* — a moving gradient across a pill only a few characters wide is barely legible as motion; (3) *system coherence* — the reduced-motion fallback above already freezes both sweep carriers to a static solid gold fill, so a deliberately-solid gold pill is an already-accepted "In focus" signal, not a new visual language. The animated members of the In-focus family (`.gold-sweep`, `.in-focus-frame`) are unchanged; the pill is a third, deliberately-static member.
...
**Shipped (design-system-t4b9k, active tab reverted by design-system-grtw7):**
- [x] **Sidebar nav (desktop rail — layered, dir 3a burgundy active tab)** (§ 4) — top/bottom grouped rail (`mt-auto` bottom pin), tagline under the wordmark, dir 3a item metrics, burgundy active-tab fill + gold inset-left bar + gold icon (`--color-nav-active-fill`, `--ring-active`, `--color-gold`; ADR-0014 supersedes ADR-0013's ivory placard + concave corner-notch). `.nav-item*` (`index.css`) + `DesignSystem.navItemClass`/`navItemIconClass`/`navItemActiveIconClass`/`navGroupTop`/`navGroupBottom`/`navTagline` (`DesignSystem.fs`), `Components/Sidebar.fs`, live StyleGuide specimen.
...
**Shipped (design-system-fq3vp):**
- [x] **Compact on-poster "✦ Focus" pill** (§ 4 Poster grid) — `DesignSystem.inFocusPill` / `.in-focus-pill`, a genuinely separate composition from `statusBadge InFocus`, top-left over the artwork, solid gold fill (no `.gold-sweep`, no new keyframe). Live StyleGuide specimen composes it with `inFocusFrame` on a poster to show the intended poster-grid pairing (one moving element per poster). § 4 Motion discipline documents the solid-not-swept rationale.
...
- [x] **Sidebar nav implemented** (design-system-t4b9k, 2026-07-03): top/bottom grouped rail, ivory active tab (dark-burgundy ink, gold icon), concave corner-notch boundary — shipped in code (`index.css`, `DesignSystem.fs`, `Components/Sidebar.fs`) with a live StyleGuide specimen and reflected here (§ 4, ADR-0013).
- [x] **Sidebar nav — dir 3a active-tab revert** (design-system-grtw7, 2026-07-03): active tab reverted from the ADR-0013 ivory placard + concave corner-notch back to dir 3a's own burgundy fill + gold inset-left bar + gold icon (ADR-0014 supersedes ADR-0013); tagline, item metrics, and the smaller bottom-group scale landed alongside — shipped in code (`index.css`, `DesignSystem.fs`, `Components/Sidebar.fs`) with a live StyleGuide specimen and reflected here (§ 4). `npm run build` compiles clean.
...
- [x] **Compact on-poster "✦ Focus" pill shipped** (design-system-fq3vp, 2026-07-03): `DesignSystem.inFocusPill` / `.in-focus-pill` — the compact 3c grid-badge variant, a solid gold fill deliberately kept separate from the animated `.gold-sweep`/`inFocusFrame` pair (motion economy — one moving element per poster) — shipped in code (`index.css`, `DesignSystem.fs`) with a live StyleGuide specimen composing it with `inFocusFrame`, and reflected here (§ 4 Poster grid, § 4 Motion discipline). `npm run build` compiles clean.
