...
**Reads compose the cache and views at query time, never at the API layer** (ADR-0048): `SeriesProjection.getAll`/`getBySlug`/`getRecentSeries`/`getRecentlyFinished`/`getRecentlyAbandoned`/`getDashboardSeriesNextUp` `LEFT JOIN` `series_metadata_cache` (TmdbRating/Overview/EpisodeRuntime) and the `series_next_up`/`series_episode_counts` views (NextUp/SeasonCount/EpisodeCount) directly inside their own query functions — every Shared DTO keeps its existing shape, and the internal `Api.fs` callers that read fields off `getBySlug`'s DTO to drive their own logic see the same fully-populated shape they always did. A cache miss degrades gracefully (`None`/`""`/empty seasons), never a synchronous TMDB fetch on the read path. `recalculateProgress` now only maintains `watched_episode_count`; `next_up_*` lives solely in the view. One known gap from this cutover is backlogged, not yet fixed: `series_metadata_cache` has no write path going forward (only a one-time seed — `series-t3jkv`). `getDashboardSeriesNextUp` additionally composes three per-season/per-episode watch-state fields the same way (`series-ww1rb`): `CurrentSeasonNumber` (the Next Up episode's season, or the highest-numbered cached season when there is no Next Up), `CurrentSeasonWatched` (one flag per episode of that season, in order, `true` when watched in ANY rewatch session — holes preserved, never collapsed to a count), and `SeasonsTouched` (one flag per season, `true` when it has ≥1 watched episode anywhere — two states only, a fully-watched season reads the same as a partially-watched one). Structure comes from `series_episode_cache`, watch state from `series_episode_progress`, gathered as two grouped queries over the whole dashboard slug set rather than a per-series round trip. On the client, the Dashboard's Series-tab "Next Up" list (`seriesNextUpItemEnhanced`) moved its season-rail + episode-dots row out from under the poster thumbnail and into the text column beside it — the thumbnail is only 40x56px, too small to carry two legible rows, so the old single 4px continuous-percentage overlay bar (which could only ever paint a prefix) was retired outright rather than shrunk further.

**`series_list`/`series_detail` physically no longer carry the externally-sourced columns** (ADR-0051, `series-d5tpn`, the proof step of the ADR-0043–0051 chain): `tmdb_rating`/`season_count`/`episode_count`/`next_up_season`/`next_up_episode`/`next_up_title` are dropped from `series_list`, and `overview`/`tmdb_rating`/`episode_runtime`/the vestigial `jellyfin_id` are dropped from `series_detail` — `ALTER TABLE ... DROP COLUMN`, run once at startup after the metadata-cache seed. `status` and `backdrop_ref` are the two exceptions that stay: `status` because the narrowed `Series_refreshed` still carries every real transition into the log, `backdrop_ref` because ADR-0048 already classified it an identity-card field (`Series_backdrop_replaced`-driven, never TMDB-refreshed) — the task's own text proposed dropping it too, which would have broken that event's handler; ADR-0051 records the correction. `Administration.checkProjectionDrift` reports zero discrepancies for `SeriesProjection` as a result: drift reaches zero by removing the columns, never by an ignore-list on `Administration.diffTable`. The now-unreachable lossy-rebuild guard (`administration-kv7dp`, ADR-0049) is fully retired — `SeriesProjection.dropTables` no longer drops the `series_season_cache`/`series_episode_cache` Cache-tier tables either, so a rebuild is safe.
