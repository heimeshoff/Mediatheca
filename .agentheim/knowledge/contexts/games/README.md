...
- **Description sanitizer** (games-r1tx4) — one shared HTML-subset sanitizer (`DescriptionSanitizer.sanitize`, lifted out of `Audible.fs`; allowlist `p`/`br`/`b`/`strong`/`i`/`em`/`ul`/`ol`/`li`, attributes always dropped, every other tag unwrapped to its own text) is shared by `Audible.fs`, `Steam.fs` (sanitizes `about_the_game`/`detailed_description` at decode time via `Steam.storeDescription`), and `Rawg.fs`. Its sanitized output only ever lands in the `game_metadata_cache` identity card (`MetadataCache.upsertGameIdentityCard`), written imperatively on every creation path (RAWG's `addGame`, and all Steam creation sites: `runSteamFamilyImport`'s new-game branch, `addGameFromSteamCore`, the Steam library import's new-game branch, and `PlaytimeTracker`'s scheduled-sync new-game path) — never the `ProjectionHandler` (ADR-0045). The `Game_added_to_library` event payload's `Description` field stays plain text on every one of those paths — Steam sites project the sanitized HTML down through `DescriptionSanitizer.toPlainText` (turns `<br>`/`</p>`/`</li>` into a newline, drops every other allowed tag, collapses repeat newlines, trims) before it rides the event; RAWG already wrote the plain `description_raw`. No HTML ever rides an event (ADR-0043), on either source. `src/Client/Pages/GameDetail/Views.fs` renders both `game.Description` and `game.ShortDescription` (cache-sourced) through `RichText.render` instead of `prop.text`.
- **Description backfill** (games-fffvm, ADR-0043/ADR-0045/ADR-0081) — a fourth resumable throttled backfill (mirroring `GameFacetBackfill.fs`/`GameDeckCompatBackfill.fs`/`GameReleaseDateBackfill.fs`) that re-fetches every already-cached game's description through games-r1tx4's sanitizer: Steam-linked via the storefront (`Steam.storeDescription`, already sanitized at decode time), RAWG-only via RAWG's game-details endpoint (`Rawg.getGameDetails`, sanitized by the job itself — `Description` falling back to `DescriptionRaw`). Walks its OWN `description_fetched_at` cursor (`MetadataCache.findGamesNeedingDescriptionBackfill`, a LEFT JOIN — the RAWG-only legacy population games-v4nqe's seed never created a row for is still a candidate), stamped by `MetadataCache.stampDescriptionFetched` exactly when a sanitizer-output description is genuinely written — never on a short_description/website_url-only refresh (`Api.fs`'s `updateGameIdentityCache` stamps iff its `description` parameter is `Some`; `upsertGameIdentityCard` itself stays stamp-free since it's shared with description-less refreshes). Steam wins when a game has both a `steam_app_id` and a `rawg_id`; both ids are read from `game_detail`, never `game_metadata_cache`'s own stale one-time-seeded `rawg_id`. A blank RAWG API key skips a RAWG candidate without an HTTP call, counted separately in `BackfillResult.Skipped` (neither an error nor a success). Registered as the `"Game description backfill"` `JobSpec` at `description_backfill_hour` (default 08:00 local, an hour clear of the release-date backfill's 07:00).
- **Deck-compat retry backoff** (games-wkyf0, ADR-0084) — a fetch that returns `Error` (store page without the `data-hardwarecompatibility` attribute — delisted apps, age-gate redirects, tools/DLC) or throws no longer gets retried every single night forever. `game_metadata_cache.deck_compat_failed_attempts`/`deck_compat_last_attempt_at` (bookkeeping only — `deck_compat`/`deck_compat_fetched_at` above stay untouched by a failure) drive `MetadataCache.findGamesNeedingDeckCompatBackfill`'s own backoff: a game with N failed attempts is skipped until `min(2^N, 30)` days have passed since its last attempt (2, 4, 8, 16, then a flat 30-day ceiling), and every failure kind — a Steam `Error` result and an unhandled exception alike — backs off the same way on purpose (ADR-0084: one rule is easier to pin than an unevidenced transient/permanent classifier, and the badge is never time-critical). A genuine success (`MetadataCache.upsertGameDeckCompat`) resets both bookkeeping columns; ADR-0084 amends ADR-0059's 'same retry semantics as GameFacetBackfill' claim, which no longer holds for this job. The backfill takes its clock as a parameter (`GameDeckCompatBackfill.runBackfill`'s `now`), so the backoff is testable without sleeping; `BackfillResult` and the scheduled job's summary line report a failed fetch (`Failed`) separately from a success and an exception (`Errors`).
- **Deck-compat age-based re-check** (games-kfpqp, ADR-0087) — a second cohort in `MetadataCache.findGamesNeedingDeckCompatBackfill`, next to the never-fetched one: a game whose recorded verdict's age exceeds a per-verdict limit (Unknown 30 days, Playable/Unsupported 90, Verified 180 — shorter for the verdict most likely still in flux, longer for a confirmed-working game least likely to regress) is re-fetched so a Valve verdict change (Unknown becoming Playable/Verified, a Playable-to-Verified bump, a downgrade) reaches an already-stamped game — until now nothing ever revisited a game once `deck_compat_fetched_at` was stamped. Capped at 25 due re-checks per run, oldest `deck_compat_fetched_at` first, counted only after games-wkyf0's failed-attempt backoff filter (a due re-check still inside its backoff never consumes a cap slot) — the never-fetched cohort stays uncapped, so a newly added game is still picked up the next morning. A successful re-check overwrites `deck_compat` and re-stamps `deck_compat_fetched_at` via `upsertGameDeckCompat`, which now takes the job's injected `now` instead of stamping `DateTime.UtcNow` itself, so the stamp and the age-limit comparison share one clock; a failed re-check goes through the same `recordDeckCompatFailedAttempt` backoff as a never-fetched game, leaving the recorded verdict/stamp untouched. `BackfillResult` gains `Rechecks` (candidates from the re-check cohort) and `VerdictsChanged` (successful re-checks whose verdict differs from the one on record), both broken out in the scheduled job's summary line alongside first fetches, `Failed`, and `Errors`. A stored `deck_compat` that is NULL or unrecognised decodes as Unknown for age-limit purposes. The intervals and the cap are plain constants next to the cursor query, not settings — cheap to tune once real Valve verdict-change data exists.
