...
- **Finished on** — the date of the `Book_status_changed Finished` event (event timestamp, or a source's own effective-on date per ADR-0077, e.g. a manual back-dated finish or an Audible `is_finished` import). Drives the dashboard's 7-day "just finished" linger (the intelligence-b1nz5 pattern).
