...
Fixed the structural fault ADR-0073 §3/§4 assumed but the code didn't actually have: `growingTabArea` (`src/Client/Pages/Dashboard/Views.fs`) rendered two entirely different, unkeyed React trees for `Expanded = None` (a flat list of collapsed cards) and `Expanded = Some` (an `invisible`-wrapped copy of the same cards plus the grown surface), so React reconciled them positionally on every expand and collapse — remounting collapsed cards (replaying `chromeClass`'s `animate-fade-in-up` mount entrance, symptom 1) and, depending on how the array lengths lined up, repurposing a surviving DOM node's identity for a different card at the same index (symptom 2's category).

The fix makes both states render the same two keyed siblings:
- The collapsed content wrapper (`prop.key "content"`) is now built once and rendered in both branches — only its `className`/`aria-hidden` change between "visible flex list" and "invisible grid-placed layer".
- The grown surface (`prop.key "surface"`, added inside `expandable`'s `expanded` closure, on the `Html.div` carrying `State.expandedCardElementId`) is present only when a card is grown.
- The outer container is `grid grid-cols-1` unconditionally now (previously it switched between `flex flex-col` and `grid grid-cols-1`), so the content wrapper's grid-placement classes are meaningful even before anything is ever expanded.
- With both siblings keyed, React mounts/unmounts the surface on its own and never touches the content wrapper's subtree — every collapsed card's DOM node (and everything under it, including its `[data-flip-key]` items) now survives every expand and every collapse.
- Added a code comment at the snapshot call site (`snapshotAndDispatch`) documenting the consequence the task's Notes called out: with the collapsed cards now genuinely persisting, the before-snapshot taken at collapse time sees every surviving item's `[data-flip-key]` twice (hidden clone, then visible surface); `Motion.Flip.snapshot`'s `Map.ofArray` keeps the *last* entry, i.e. the surface's box, which is the correct "from" for the collapse travel — flagged so a future reader doesn't "fix" this.
- `chromeClass` (and its `animate-fade-in-up`) is untouched — it now only fires on a tab's genuine first mount, since the cards it's attached to no longer remount on expand/collapse.
- `src/Client/Motion.fs` was not touched, per the task's explicit acceptance criterion.

**Symptom 2's DOM mechanism — what was found and the limits of what could be confirmed.** The general index-based-reconciliation defect was reproduced directly (see the new e2e test below) and the fix confirmed to remove it. The *exact* Movies-tab "Recently Added shows Recently Watched's large tiles" artifact could not be reproduced end-to-end in this sandbox: `addMovie` requires a TMDB lookup, and this dev environment has no TMDB API key configured (confirmed via the server's own boot log: "Skipping nightly refresh: TMDB API key not configured"), so real movie items could not be seeded into `MoviesRecentlyWatched`/`MoviesRecentlyAdded`. The *empty*-data case was probed on the pre-fix code (expand `MoviesRecentlyWatched`, collapse, inspect `#dashboard-collapsed-card-MoviesRecentlyAdded`): with both cards empty, the collapsed `MoviesRecentlyAdded` node came back completely correct after the round trip (`outerHTML` showed the right header, right icon, right "No recently added movies" body). The two empty-state subtrees happen to be shape-identical enough (both a lone `emptyNote` `div` under the same header shape) that React's positional diff lines up by accident when there is no real content — the visible corruption needs actual poster tiles in the mismatched positions to produce the "large tiles at the top, real content pushed below" shape the builder described. This is consistent with the task's own framing ("the remap is the confirmed root and the fix below removes it regardless"); the same root mechanism was confirmed hermetically on the Games tab instead, where `addGame` needs no external API.

**New regression test** (`tests/e2e/dashboard-card-collapse-persists.spec.ts`, Playwright): seeds a game via `addGame` (hermetic, same pattern as `game-detail-persistent-cards.spec.ts`), captures the seeded poster tile's `[data-flip-key]` element by reference, expands and collapses `GamesRecentlyAdded`, and asserts the tile is still the same DOM node (`===`) and still `isConnected`. TDD red/green verified by hand: with the fix reverted, the test fails at `expect(stillConnected).toBe(true)` (`isConnected` was `false` — a genuine remount, not a flaky assertion); with the fix restored, it passes. This directly exercises acceptance criterion 2's DOM-identity check, which was not marked `[human-eye]`.

Verification (runner exit status; independently re-run by the verifier from the worktree): `npm run build` exit 0 (`✓ built in 37.73s`, only the pre-existing, unrelated FS0020 warning in `Pages/AdminProjections/Views.fs`); `npm test` exit 0 — 769 Expecto tests passed, 0 failed; `npm run test:client` exit 0 — 61 Vitest tests passed / 10 files, including the untouched `Pages/Dashboard/ExpandCard.test.fs` (10) and `Motion.test.fs` (8); `npx playwright test tests/e2e/dashboard-card-collapse-persists.spec.ts` exit 0 — 1 passed (41.1s), cold-started against an isolated temp `DATA_DIR` so no live DB was touched.

**Verifier structural confirmations** (iteration 1 PASS): `prop.ref setContainerRef` now sits on the *outer* container in both states, so the FLIP snapshot root still contains every `[data-flip-key]` in the tab; `#dashboard-expanded-card` remains a **sibling** of the hidden clone, never an ancestor, so `playRoot`'s scoping still excludes the clone on expand and `root` still has no duplicates on collapse — the exact class of defect that failed `intelligence-m09d4`'s iteration 1 is not reintroduced.

**Residual risk flagged by the verifier for the builder's eye-check:** the collapsed state's outer container changed from `flex flex-col gap-4` to `grid grid-cols-1`, with the `flex-col` moved one level down into the keyed wrapper. The `minmax(0, 1fr)` track floor now applies when nothing is expanded too, and the horizontal poster rails sit one level deeper inside a grid item (`min-width: auto`). Layout rendered correctly in the e2e run, but collapsed-state rail width is worth a glance.

**Builder eye-checks outstanding** (the three `[human-eye]` criteria — never a worker or verifier action): (1) collapsing any dashboard card — the cards that were on screen before the expand reappear in place with no fade-in-up entrance, checked on the All tab and the Movies tab; (2) Movies tab — expand Recently Watched, let the grow finish, collapse; Recently Added shows only its own header and list, Recently Watched only its own poster rail, both at normal collapsed height, and the same when the pair is reversed; (3) rapid expand/collapse toggling (collapse mid-grow, expand mid-shrink) leaves no card with stale content, stale height, or a stuck transform.

Key files: `src/Client/Pages/Dashboard/Views.fs` (`growingTabArea`, `expandable`'s `expanded` closure, `snapshotAndDispatch`'s comment), `tests/e2e/dashboard-card-collapse-persists.spec.ts` (new).
