...
- `ExpandedItemsLoaded` does not animate: the unlimited list extends the limited one in
  ...
  do not move again.
- Move the scroll-into-view into the same layout effect, ahead of `play`, where React
  guarantees it runs post-commit. Delete `State.scrollToExpandedCardCmd`, its 50ms
  `setTimeout`, and the `Cmd.batch` in `ExpandCard`. No shape change to `Model`,
...
- [ ] Expanding a poster-rail card (e.g. Movies Recently Added): the card grows and the already-visible posters travel smoothly, without a flash or jump, to their expanded grid positions in roughly half a second; the posters that arrive from the unlimited fetch appear below without disturbing the ones that moved. [human-eye]
