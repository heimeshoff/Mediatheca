...
Implemented ADR-0079 (catalog entries typed by `MediaType`) end to end: `Catalogs.EntryAddedData`/`EntryState` gain `MediaType: MediaType option` (encoder omits the key when `None`, decoder reads it optionally — `movieSlug`/`movie_slug` unchanged); `CatalogProjection.getEntries` now joins the exact `*_list` table for a typed row (movie/series/game/book) and falls back to the original join-order inference only for `media_type IS NULL` legacy rows; `getCatalogsForMedia` (replacing `getCatalogsForMovie`), `getCatalogsForSeriesWithChildren` and `getEntriesByMediaSlug` all filter on `media_type = @type OR media_type IS NULL`, so `getCatalogsForGame` no longer aliases the movie lookup and a media removal cascade can no longer delete a same-slugged entry of another type. `catalog_entries.media_type TEXT NULL` is added via the idempotent `ALTER TABLE … ADD COLUMN … with _ -> ()` guard (`GameProjection.fs`/ADR-0055 precedent); `UNIQUE(catalog_slug, movie_slug)` is unchanged (ADR-0079 §5). Shared gets `CatalogEntryDto`/`CatalogRef`/`AddCatalogEntryRequest` renamed to `MediaSlug`/`Title`/`Year`/`PosterRef`/`MediaType` (no more `RoutePrefix` on the DTO), a `MediaType.routePrefix` helper, and `getCatalogsForBook` on `IMediathecaApi`. `Api.fs`'s `addCatalogEntry` and the three removal cascades (`removeMovie`/`removeSeries`/`removeGame`) now pass the typed `MediaType`. The three existing add-to-catalog flows (`MovieDetail`/`SeriesDetail` incl. season/episode/`GameDetail` `State.fs`) send the correct `MediaType`; `CatalogDetail/Views.fs` and `SeriesDetail/Views.fs` are field-renamed to match, with `RoutePrefix` derived via `MediaType.routePrefix` at the two render sites that still need a string (`EntryList.EntryItem`, `PosterCard.viewForRoute`). `npm run build`, `npm test` (919/919) and `npm run test:client` (103/103) are all green.

**Deviation from the literal acceptance criteria, directed by a `tactical-modeler` consult (single-specialist, ADR-0035):** while writing the projection test for "a movie and a book sharing a slug ... each resolve to their own row," I found ADR-0079 §3 (two typed entries are duplicates iff `(MediaType, slug)` match — implying two different-typed entries CAN share a slug in one catalog) is internally inconsistent with §5 (the projection's `UNIQUE(catalog_slug, movie_slug)` stays slug-only, unwidened). Accepting such an add at the aggregate level produces an `Entry_added` event whose projection write (`INSERT OR REPLACE`) silently clobbers the earlier same-slugged entry regardless of type — a replay-breaking divergence, confirmed by running the scenario. I consulted `agentheim:tactical-modeler` directly with the contradiction; the ruling: §5 wins until the constraint is widened, so `Catalogs.decide`'s `Add_entry` now rejects **any** same-slug add within one catalog, typed or not — the `(MediaType, slug)` pair still governs resolution, `getCatalogsFor*` and the removal cascade, it just isn't the duplicate/uniqueness key yet. This means the task's literal acceptance-criteria line "accepts a same-slug add of a different type when both are typed" is now a **rejection** instead (`CatalogsTests.fs`'s renamed test: "Add_entry rejects a same-slug entry of a different media type while the projection key is slug-only (ADR-0079 §5)"), and the projection test for "movie and book sharing a slug each resolve to their own row" (`CatalogProjectionTests.fs`) is scoped **across two catalogs** rather than the same catalog, per the ruling. Every other acceptance criterion is satisfied as originally written.

**ADR-0079 amendment (not written to disk by this worker — conductor/human should append to `.agentheim/knowledge/decisions/0079-catalog-entries-typed-by-mediatype-legacy-entries-untyped.md`; no new ADR file was warranted, per the specialist):**

> **Amendment 2026-09-16 (curation-cyxbc, implementation):** §3's typed-pair duplicate rule cannot be enforced while §5 keeps `UNIQUE(catalog_slug, movie_slug)` — two typed entries of different `MediaType` sharing a slug in one catalog would be accepted by `decide` and then silently clobbered by the projection's `INSERT OR REPLACE`. Until the follow-up in §5 lands, `decide` rejects any same-slug add within a catalog, typed or not. §3's pair identity still governs resolution, type-scoped lookups (`getCatalogsFor*`) and the removal cascade (`getEntriesByMediaSlug`) — only the duplicate/uniqueness *key* is narrowed back to slug-only for now. See the follow-up backlog item (`curation-w9fkq`, reported alongside this task) for relaxing this once the constraint widens.

**Non-bullet README edits** (report only — not expressible as a bullet `replace`/`append` op, apply by hand): the Purpose paragraph's "ordered lists of movies / series / games" → "ordered lists of movies / series / games / books".

**Out of scope, left untouched, matching the task's own Notes:** `EventFormatting.formatCatalogEvent`'s `Entry_added` label (optional "may append the media type" — the nested JSON shape (`{"data": {...}, "position": ...}`) means `tryField` can't reach `movieSlug`/`mediaType` today without a nested-field decoder, a pre-existing, separate defect noted in ADR-0079's Consequences, not attempted here to stay in scope). `crossLinkFields` mislinking non-movie catalog entries — pre-existing, noted in the ADR as its own small task. The book detail page's catalog pills/`CatalogManager` extraction and `removeBook`'s cascade remain `books-f3sb2`'s, which now depends on this task as before.

Key files: `src/Server/Catalogs.fs` (event/aggregate + serialization), `src/Server/CatalogProjection.fs` (schema migration + typed joins + type-filtered lookups), `src/Server/Api.fs` (typed `addCatalogEntry`, typed removal cascades, `getCatalogsForBook`), `src/Shared/Shared.fs` (DTO renames, `MediaType.routePrefix`, `getCatalogsForBook`), `src/Client/Pages/{MovieDetail,SeriesDetail,GameDetail}/State.fs` (typed add-to-catalog), `src/Client/Pages/{CatalogDetail,SeriesDetail}/Views.fs` (field renames), `tests/Server.Tests/CatalogsTests.fs` + new `tests/Server.Tests/CatalogProjectionTests.fs`.
