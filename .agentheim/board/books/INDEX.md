...
- **books-d4wtc** — Prior reading progress — a source's first-ever position report for a book is a `Prior_reading_progress_recorded` event (never InFocus-promoting, finishing with its own date), projected as `kind = 'prior'` so the History list, the Hours Listened stat and any future Reading day never mistake an import's starting position for a listening session (feature) — `todo/books-d4wtc-prior-reading-progress-event.md`
