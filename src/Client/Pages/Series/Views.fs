...
let private inFocusBadge =
    Html.div [
        prop.className "absolute top-1.5 right-1.5 z-10 w-6 h-6 rounded-full bg-primary/80 flex items-center justify-center text-primary-content shadow-md border border-primary/30"
        prop.title "In Focus"
        prop.children [
    ...
    Html.div [
        ...
        prop.children [
            ...
            if model.IsLoading then
                ...
                let filtered =
                    if model.SearchQuery = "" then model.Series
                    else
                        let query, yearFilter = FuzzyMatch.extractYear model.SearchQuery
                        let items = model.Series |> List.map (fun s -> (s.Name, s))
                        let matched = FuzzyMatch.fuzzyFilter query items
                        match yearFilter with
                        | Some year -> matched |> List.filter (fun s -> s.Year = year)
                        | None -> matched
